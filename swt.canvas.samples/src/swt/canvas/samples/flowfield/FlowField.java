...
//		drawCircle((int) particle.x, (int) particle.y, 10);
...
//		}
	...
	private void updateFlowField(int step) {
		...
		int width = getWidth();
		int index = 0;
		for (int iy = 0; iy < height / fResolution; iy += step) {
			for (int ix = 0; ix < width / fResolution; ix += step) {
				double noise = fNoise.noise3_Classic((ix + .01) / 100, (iy + .01) / 100, thirdD);
